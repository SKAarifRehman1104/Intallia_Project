...
import { SimulationDetails, TaskCounts, getInitialTaskCounts } from "@/pages/Simulation/AddSimulation/SimulationDetails";
...
const AddSimulation = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('simulationTaskCountsUpdated', handleTaskCountUpdate as EventListener);
    
    return () => {
      window.removeEventListener('simulationTaskCountsUpdated', handleTaskCountUpdate as EventListener);
    };
  }, []);

  return (
    <MainLayout>
        <div className="bg-[rgba(248,249,250,1)] flex items-start gap-[35px] overflow-hidden flex-wrap pr-[67px] pb-[37px] max-md:pr-5">
     
     <div className="grow shrink-0 basis-0 w-fit mt-9 max-md:max-w-full">
       ...
       <div className="flex w-full max-w-[1473px] items-stretch gap-5 flex-wrap justify-between mt-[63px] max-md:max-w-full max-md:mt-10">
         <ContentSection />
